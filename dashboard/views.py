...
def index(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if(values):
                ...
                if(state):
                    ...
                    if district:
                        feedbacks = feedbacks.filter(disctrict=district)
                    block = values.get('block')
                    if block:
                        feedbacks = feedbacks.filter(name_of_block=block)
        ...
        print(male_attendance)
    return render(request, 'dashboard/home.html', { "new_visitors" : male_attendance, "returning_visitors": female_attenadance,
